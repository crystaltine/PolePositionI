import pygame
from CONSTANTS import COLOR_ACTIVE, COLOR_INACTIVE, FONT_MEDIUM, FONT_SIZES

class Input:
    """
    ### Currently, this can only be used for the main menu's room ID input box.
    
    @see - https://stackoverflow.com/a/46390412/21385219
    """
    def __init__(self, x, y, w, h, text=''):
        """
        Parameters: 
        - x: int, x position of input box
        - y: int, y position of input box
        - w: int, width of input box
        - h: int, height of input box
        - text: str, default text to display in input box (optional)
        
        x and y are the position of the **top left corner** of the input box.
        """
        self.rect = pygame.Rect(x, y, w, h)
        self.color = COLOR_INACTIVE
        self.text = text
        self.txt_surface = FONT_MEDIUM.render("ROOM ID...", True, (100, 100, 100))
        self.active = False

    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            # If the user clicked on the input_box rect.
            if self.rect.collidepoint(event.pos):
                # Toggle the active variable.
                self.active = not self.active
            else:
                self.active = False
            # Change the current color of the input box.
            self.color = COLOR_ACTIVE if self.active else COLOR_INACTIVE
        if event.type == pygame.KEYDOWN:
            if self.active:
                if event.key == pygame.K_BACKSPACE:
                    self.text = self.text[:-1]
                elif len(self.text) < 6 and event.unicode.isalnum(): # only allow alphanumeric characters, and max length of 6
                    self.text += event.unicode.upper() # all game IDs are uppercase alnums
                
                # Re-render the text.
                
                # if no text, render a faded placeholder text
                if self.text == '':
                    self.txt_surface = FONT_MEDIUM.render("ROOM ID...", True, (100, 100, 100))
                else:
                    self.txt_surface = FONT_MEDIUM.render(self.text, True, self.color)

    def update(self):
        # Resize the box if the text is too long.
        width = max(200, self.txt_surface.get_width()+10)
        self.rect.w = width

    def draw(self, screen):
        # Blit the text. Make sure to center it.
        screen.blit(self.txt_surface, (self.rect.x+10, self.rect.y + (self.rect.h/2 - FONT_SIZES["medium"]/2 - 5)))
        # Blit the rect.
        pygame.draw.rect(screen, self.color, self.rect, 2)