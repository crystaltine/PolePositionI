...
### Rendering
Since racing games are generally three-dimensional, we initially experiented with a custom-built rendering engine. However, since Pole Position was never an open-world game, much of the functionality of a 3D renderer was actually counterproductive - for example, since players are not permitted to stray too far from track boundaries, there is no reason to handle entity rendering from different angles.
